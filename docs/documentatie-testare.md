...
### Setul de teste pentru testarea pe circuite independente
...
#### Construcția mulțimii de teste

Mulțimea de teste se obține pornind de la cel mai simplu drum din program, care nu
execută nicio excepție și niciun scurtcircuit și generând câte un drum suplimentar pentru fiecare
decizie prin comutarea acelei decizii, păstrând restul parametrilor cât mai apropiați de linia de
bază. Pentru a maximiza independența liniară a setului de teste și a oferi o trasabilitate directă
între test și drum, suita asociază **fiecărui drum P1&ndash;P20 un test dedicat** (`P1_*`&ndash;`P20_*`).

**Drumul P1** (de bază): toate validările trec, `merchant` nu este în lista neagră, scorul rămâne `0` și
metoda returnează `APPROVED`. Restul drumurilor se obțin prin comutarea unei singure decizii față de
`P1`; acolo unde comutarea unei decizii o forțează pe alta (de exemplu `D9 = true` implică `D8 = true`,
fiindcă `amount > 5000` implică `amount > 1000`), testul exersează simultan drumurile dependente, dar
oracolul țintește decizia primară.

| Drum  | Decizie comutată față de baseline                          | Test asociat                           | Rezultat așteptat           |
|-------|------------------------------------------------------------|----------------------------------------|-----------------------------|
| `P1`  | &mdash; (drumul de bază)                                   | `P1_baseline`                          | `APPROVED` (scor `0`)       |
| `P2`  | `D1 = true` (`amount ≤ 0`)                                 | `P2_invalidAmount`                     | `IllegalArgumentException`  |
| `P3`  | `D2 = true` (`hour ∉ [0, 23]`)                             | `P3_invalidHour`                       | `IllegalArgumentException`  |
| `P4`  | `D3 = true` (`country ∈ {null, blank}`)                    | `P4_invalidCountry`                    | `IllegalArgumentException`  |
| `P5`  | `D4 = true` (`merchant ∈ {null, blank}`)                   | `P5_invalidMerchant`                   | `IllegalArgumentException`  |
| `P6`  | `D5 = true` (`tx24 < 0` sau `avg < 0`)                     | `P6_invalidHistory`                    | `IllegalArgumentException`  |
| `P7`  | `D6 = true` (`highRisk = null` sau `blacklist = null`)     | `P7_nullRiskSets`                      | `IllegalArgumentException`  |
| `P8`  | `D7 = true` (`merchant` blacklistat)                       | `P8_blacklistedMerchant`               | `BLOCKED` (scurtcircuit)    |
| `P9`  | `D8 = true` (`amount > 1000`)                              | `P9_amountAbove1000`                   | `APPROVED` (scor `10`)      |
| `P10` | `D9 = true` (`amount > 5000`, forțează `D8`)               | `P10_amountAbove5000`                  | `REQUIRES_2FA` (scor `30`)  |
| `P11` | `D10 = true` (`hour < 6` sau `hour > 22`)                  | `P11_nightHour`                        | `APPROVED` (scor `15`)      |
| `P12` | `D11 = true` (`newBen && amount > 3000`, forțează `D8`)    | `P12_newBeneficiaryHighAmount`         | `REQUIRES_2FA` (scor `35`)  |
| `P13` | `D12 = false` (`hasNext()` returnează `false`, `for` iese) | `P13_emptyHighRiskCountries_skipsLoop` | `APPROVED` (scor `0`)       |
| `P14` | `D13 = true` (potrivire în `for` &rarr; `break`)           | `P14_highRiskCountryMatch`             | `APPROVED` (scor `0`)       |
| `P15` | `D14 = true` (`isHighRisk && amount > 3000`)               | `P15_highRiskCountryAndHighAmount`     | `REQUIRES_2FA` (scor `40`)  |
| `P16` | `D15 = true` (`tx24 > 10`)                                 | `P16_manyTransactions`                 | `APPROVED` (scor `20`)      |
| `P17` | `D16 = true` (`avg > 0 && amount > 3·avg`)                 | `P17_amountExceedsAverageMultiple`     | `APPROVED` (scor `25`)      |
| `P18` | `D17 = true` (`scor ≥ 90`)                                 | `P18_extremeRiskScore`                 | `BLOCKED` (scor `145`)      |
| `P19` | `D18 = true` (`60 ≤ scor < 90`)                            | `P19_highRiskScore`                    | `MANUAL_REVIEW` (scor `70`) |
| `P20` | `D19 = true` (`30 ≤ scor < 60`)                            | `P20_mediumRiskScore`                  | `REQUIRES_2FA` (scor `30`)  |
...
Pentru a izola direct comutarea pe `D12`, testul `P13_emptyHighRiskCountries_skipsLoop` folosește
scenariul (1) cu `highRiskCountries = Set.of()`. Scenariul (2) este exersat colateral de toate testele
care folosesc `country = "RO" ∉ highRiskCountries` (de exemplu `P1_baseline`, `P9_amountAbove1000`).

#### Cardinalitatea setului

Cele `20` de drumuri liniar independente sunt acoperite de o suită de **20 de teste**
(`P1_*`&ndash;`P20_*`) &ndash; câte unul per drum, în corespondență directă `1:1`. Astfel se pot vedea
ușor următoarele avantaje:

- diagnosticabilitate: dacă un test pică, drumul defect este identificat unic prin numele
  metodei (`P<i>_*`), fără analiză suplimentară;
- independență liniară explicită: fiecare test comută o singură decizie față de baseline (sau
  cel mai mic număr forțat de implicații logice între predicate, ex. `D9 → D8`).
